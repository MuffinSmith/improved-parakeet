...
for index, x in enumerate(providers):
       DNS_dictionary[x] = ips[index]
...
DNS_dictionaries = []
for index, x in enumerate(providers):
       DNS_dictionaries.append({'provider_name': x, 'primary_server': ips[index]})
...
for x in secondary_ips:
    for y in DNS_dictionaries:
        if(x["provider"]==y["provider_name"]):
            y["secondary_server"] = x["ip"]
            break